...
class ActivityTransferDone : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        isCalllogs = intent.getBooleanExtra(MyConstants.CALLLOGS, false)
        isCalendar = intent.getBooleanExtra(MyConstants.CALENDAR, false)
        isContacts = intent.getBooleanExtra(MyConstants.CONTACTS, false)
        isPictures = intent.getBooleanExtra(MyConstants.PICS, false)
        isVideos = intent.getBooleanExtra(MyConstants.VIDEOS, false)
        isApps = intent.getBooleanExtra(MyConstants.APPS, false)
        isAudios = intent.getBooleanExtra(MyConstants.Audios, false)
        isDocs = intent.getBooleanExtra(MyConstants.Docments, false)
    ...
    fun storeOtherData(m: Int) {
        ...
        if (isPictures) {
            ...
            }, multiplier * MyConstants.animationTime)
            multiplier++
        }
        if (isVideos) {
            Handler().postDelayed({
    ...
    fun saveContacts() {
        val intent = Intent(this, ActivityStoreContact::class.java)
        startActivityForResult(intent, MyConstants.STORE_CONTACTS)
    }


    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        if (requestCode == MyConstants.STORE_CONTACTS) {

            if (data!!.hasExtra(MyConstants.isContactsSaved)) {
                isContactsSaved = data.getBooleanExtra(MyConstants.isContactsSaved, false)
            }
            ...
            if (isApps) {
                Handler().postDelayed({
                    showSaveAppsDialogue()
                }, 1 * MyConstants.animationTime)
            } else {
                if (isAudios) {
                    Handler().postDelayed({
                        findViewById<CardView>(R.id.cl_audios).visibility = View.VISIBLE
                    }, 2 * MyConstants.animationTime)
                }
                if (isDocs) {
                    Handler().postDelayed({
                        findViewById<CardView>(R.id.cl_docs).visibility = View.VISIBLE
        ...
        } else if (requestCode == MyConstants.INSTALL_APPS) {
    ...
    fun showSaveContactsDialogue() {
        ...
        negative.setOnClickListener {
            ...
            if (isApps) {
                Handler().postDelayed({
                    showSaveAppsDialogue()
                }, m++ * MyConstants.animationTime)
            } else {
                if (isAudios) {
                    android.os.Handler().postDelayed({
                        findViewById<CardView>(com.phoneclone.data.R.id.cl_audios).visibility =
                            android.view.View.VISIBLE
                    }, m++ * com.phoneclone.data.constants.MyConstants.animationTime)
    ...
    fun showSaveAppsDialogue() {
        ...
        positive.setOnClickListener {
            ...
            val intent = Intent(this, ActivityInstallApps::class.java)
            startActivityForResult(intent, MyConstants.INSTALL_APPS)
        }
        negative.setOnClickListener {
            dialog.dismiss()
    ...
    private fun rateIntentForUrl(url: String): Intent? {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            intent = Intent(
                Intent.ACTION_VIEW,
                Uri.parse(
                    url + Objects.requireNonNull(this@ActivityTransferDone).packageName
                )
            )
        }
    ...
    fun loadFbBannerAdd() {
        ...
        val adView = AdView(
            this@ActivityTransferDone,
            this@ActivityTransferDone.resources.getString(R.string.banner_add),
    ...
    fun admobBanner() {
        ...
        val mAdView = com.google.android.gms.ads.AdView(this@ActivityTransferDone)
        val adSize: com.google.android.gms.ads.AdSize =
            IntersitialAdHelper.getAdSize(this@ActivityTransferDone)
        ...
        override fun doInBackground(vararg p0: Void?): Void? {
            ...
            try {
                ...
                while (line != null) {
                    ...
                    if (tokens.size > 0) {
                        var duration =
                            if (tokens[CalendarEventsModel.DURATION_INDEX] == null || tokens[CalendarEventsModel.DURATION_INDEX].contains(
                                    "null"
                                )
                            ) {
                                "PT1H"
                            } else {
                                tokens[CalendarEventsModel.DURATION_INDEX]
                            }
                        val values = ContentValues()
                        values.put(
                            CalendarContract.Events.DESCRIPTION,
                            tokens[CalendarEventsModel.DESCRIPTION_INDEX]
                        )
                        values.put(
                            CalendarContract.Events.DTSTART,
                            tokens[CalendarEventsModel.START_INDEX]
                        )
                        values.put(
                            CalendarContract.Events.EVENT_TIMEZONE,
                            tokens[CalendarEventsModel.TIME_ZONE_INDEX]
                        )
                        values.put(
                            CalendarContract.Events.TITLE,
                            tokens[CalendarEventsModel.TITLE_INDEX]
                        )
