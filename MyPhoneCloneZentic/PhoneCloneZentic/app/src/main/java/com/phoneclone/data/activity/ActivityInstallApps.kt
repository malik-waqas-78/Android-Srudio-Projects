...
import com.phoneclone.data.adapters.AdapterApps
...
class ActivityInstallApps :AppCompatActivity() {

    var appsList=ArrayList<AppsModel>()

    var rc_contacts: RecyclerView?=null

    var adapter_install_apps: AdapterApps?=null
    ...
    inner class LoadData : AsyncTask<Void, Void, Void>() {
        ...
        override fun onPostExecute(result: Void?) {
            ...
            if (this@ActivityInstallApps.isDestroyed || this@ActivityInstallApps.isFinishing) {
                return
            }
            ...
            if (appsList.size > 0) {
                ...
                var linearLayoutManager = LinearLayoutManager(this@ActivityInstallApps)
                linearLayoutManager.orientation = LinearLayoutManager.VERTICAL
                rc_contacts?.layoutManager = linearLayoutManager
                //create adapter
                adapter_install_apps = AdapterApps(this@ActivityInstallApps, appsList)
                //connect adapter to recycleView
                rc_contacts?.adapter = adapter_install_apps
            } else {
                findViewById<ConstraintLayout>(R.id.cl_progress).visibility=View.GONE
                findViewById<ConstraintLayout>(R.id.cl_appslist).visibility=View.GONE
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(requestCode== MyConstants.INSTALL_APPS){
            LoadData().execute()
        }
    }
    ...
    fun exitorcontinue() {
        ...
        negative.setOnClickListener {
            setResult(MyConstants.INSTALL_APPS)
            finish()
        }
    ...
    fun loadFbBannerAdd() {
        ...
        val adView = AdView(
            this@ActivityInstallApps,
            this@ActivityInstallApps.resources.getString(R.string.banner_add),
    ...
    fun admobBanner() {
        ...
        val mAdView = com.google.android.gms.ads.AdView(this@ActivityInstallApps)
        val adSize: com.google.android.gms.ads.AdSize = IntersitialAdHelper.getAdSize(this@ActivityInstallApps)
