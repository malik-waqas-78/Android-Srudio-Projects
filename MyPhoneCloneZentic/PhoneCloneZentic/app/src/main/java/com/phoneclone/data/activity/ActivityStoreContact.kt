...
import com.phoneclone.data.adapters.AdapterContacts
...
class ActivityStoreContact : AppCompatActivity()  {
    ...
    var adapter_Contacts:AdapterContacts?=null
   ...
   companion object get{
    ...
    inner class asyncTask : AsyncTask<Unit,Unit,Unit>(){
        ...
        override fun onPreExecute() {
            super.onPreExecute()
           progressDialog = ProgressDialog(this@ActivityStoreContact)
        ...
        override fun onPostExecute(result: Unit?) {
            ...
            if(this@ActivityStoreContact.isDestroyed||this@ActivityStoreContact.isFinishing){
                return
            }
    ...
    inner class LoadContactsFile() : AsyncTask<Void, Void, Void>() {
                ...
                while (line != null) {
                    ...
                    if (tokens.size > 0) {
                        var contact = ContactsModel(
                            tokens[ContactsModel.NAME_INDEX],
                            tokens[ContactsModel.PHONE_NUMBER_INDEX]
                        )
        ...
        override fun onPostExecute(result: Void?) {
            ...
            if (contactsList.size > 0) {
                ...
                var linearLayoutManager = LinearLayoutManager(this@ActivityStoreContact)
                linearLayoutManager.orientation = LinearLayoutManager.VERTICAL
                rc_contacts?.layoutManager = linearLayoutManager
                //create adapter
                adapter_Contacts = AdapterContacts(this@ActivityStoreContact, contactsList)
                //connect adapter to recycleView
                rc_contacts?.adapter = adapter_Contacts
                ...
                btn_saveContacts?.setOnClickListener {
                    contactsList = adapter_Contacts?.contactsList!!
                    asyncTask().execute()
                }
...
//                "Are you sure you want to Allow permissions?")
        positive.setOnClickListener {
            dialog.dismiss()
            val intent=Intent()
            intent.putExtra(MyConstants.isContactsSaved,isContactsSaved)
            setResult(MyConstants.STORE_CONTACTS,intent)
            finish()
        }
...
//                "Are you sure you want to Allow permissions?")
   ...
   /* fun loadFbBannerAdd() {
    ...
    fun admobBanner() {
        ...
        val mAdView = com.google.android.gms.ads.AdView(this@ActivityStoreContact)
        val adSize: com.google.android.gms.ads.AdSize = IntersitialAdHelper.getAdSize(this@ActivityStoreContact)
