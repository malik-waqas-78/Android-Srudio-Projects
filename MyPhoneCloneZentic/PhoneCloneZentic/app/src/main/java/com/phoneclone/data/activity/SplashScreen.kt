...
import com.phoneclone.data.utills.AppVersionChecker
...
class SplashScreen : AppCompatActivity(), DialogueClickListener,AppVersionChecker.VersionCallBack {
    ...
    //lateinit var versionChecker:AppVersionChecker
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       // versionChecker = AppVersionChecker(this)
        //MediationTestSuite.launch(this@SplashScreen);
        if(isAppInstalledFromPlay(this)){
            IntersitialAdHelper.loadStartEndAdmobInterstitial(this@SplashScreen)
            IntersitialAdHelper.loadAdmobInterstitial(this@SplashScreen)
            NativeAdHelper.loadAdmobNativeAd(this@SplashScreen)
            loadFbBannerAdd()
        }
        /*NativeAdHelper.loadAd(this@SplashScreen)*/
        ...
        /*  var dialog=MyDialogueBoxes(this@SplashScreen,this)
          dialog.transferCompleted()*/

        //start button
        iv_start?.setOnClickListener {
            //ask for permissions
            /*myPermissions = MyPermissions(applicationContext, this@SplashScreen);
            if (!myPermissions!!.haveSplashPermissions()) {
                myPermissions?.getSplashPermissions()
            } else {*/
                launchChooseMobile()
           /* }*/
        }
 ...
 //         }
   ...
   /* @RequiresApi(Build.VERSION_CODES.M)
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == MyConstants.LOCATION_PERMISIION_REQUEST_CODE || requestCode == MyConstants.STORAGE_PERMISSION || requestCode == MyConstants.ANDROIDR_PERMISSION) {
            if (myPermissions!!.isLocationPermissionGiven() && myPermissions!!.isStoragePermissionGiven()*//*&&myPermissions!!.hasAndroidRStoragePermission()*//*) {
                launchChooseMobile()
            } else {
                //location permission is denied or storage is denied
                if (requestCode == MyConstants.LOCATION_PERMISIION_REQUEST_CODE && shouldShowRequestPermissionRationale(
                        myPermissions!!.accessFineLocation
                    )
                ) {
                    val myDialogueBoxes = MyDialogues(this@SplashScreen)
                    ...
                    myDialogueBoxes.showRational(myPermissions!!.accessFineLocation, title, msg)
                } else if (requestCode == MyConstants.STORAGE_PERMISSION && shouldShowRequestPermissionRationale(
                        myPermissions!!.storageReadPermission
                    )
                ) {
                    val myDialogueBoxes = MyDialogues(this@SplashScreen)
                    ...
                    myDialogueBoxes.showRational(myPermissions!!.accessFineLocation, title, msg)
                } else if (requestCode == MyConstants.ANDROIDR_PERMISSION) {
    ...
    override fun onRequestPermissionsResult(
        ...
        if (permissions.isNotEmpty() && grantResults.isNotEmpty()) {
            if (myPermissions!!.isLocationPermissionGiven() && myPermissions!!.isStoragePermissionGiven()) {
                *//* if(myPermissions!!.hasAndroidRStoragePermission()){*//*
                launchChooseMobile()
                *//*   }else{
                       if(Build.VERSION.SDK_INT>=30){
                           ...
                           val myDialogueBoxes=MyDialogueBoxes(this@SplashScreen)
    ...
    private fun launchChooseMobile() {
        var intent = Intent(this@SplashScreen, ActivityChooseDevice::class.java)
