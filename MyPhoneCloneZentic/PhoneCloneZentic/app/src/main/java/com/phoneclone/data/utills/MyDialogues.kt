...
import com.phoneclone.data.activity.ActivityChooseDevice
import com.phoneclone.data.interfaces.DialogueClickListener


class MyDialogues(var activity: Activity) {
...
//                "Are you sure you want to Allow permissions?")
    ...
    companion object {
        fun updateToNewVersion(
            ...
            title: String,
            msg: String,
            clickListener: ActivityChooseDevice.OpenSettingsForPermissions
        ) {
    ...
    fun showRational(
        ...
        title: String,
        msg: String,
        callBacks: MyPermissions.ExplanationCallBack
    ) {
        if (!activity.isFinishing) {
            val dialog = Dialog(activity)
            dialog.setCancelable(false)
        ...
        fun showPermissionsRequired(activity: Activity, msg: String,callBacks:MyPermissions.PermissionsDeniedCallBack) {
            if (!activity.isFinishing) {
                val dialog = Dialog(activity)
                dialog.setCancelable(false)
                dialog.setContentView(R.layout.db_permissions_rationals)
    ...
    fun showRational(
        ...
        title: String,
        msg: String,
        callBacks: MyPermissions.RationalCallback
    ) {
        if (!activity.isFinishing) {
            val dialog = Dialog(activity)
            dialog.setCancelable(false)
