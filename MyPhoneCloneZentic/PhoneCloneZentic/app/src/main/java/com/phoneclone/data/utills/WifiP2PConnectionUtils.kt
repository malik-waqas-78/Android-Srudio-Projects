...
import com.phoneclone.data.interfaces.DirectCallBack

import com.phoneclone.data.interfaces.WifiP2PCallBacks
import android.os.Handler;
import android.system.ErrnoException
import com.phoneclone.data.activity.ActivityReceiveData
import com.phoneclone.data.activity.ActivitySendData
import java.io.*
import java.lang.Exception
import java.net.*


class WifiP2PConnectionUtils(var context: Context) {
    private var wifiP2pManager: WifiP2pManager? = null
    val wifiP2PActionListener = context as WifiP2PCallBacks
    ...
    val directActionListener: DirectCallBack = object :
        DirectCallBack {
    ...
    init {
        ...
        channel = wifiP2pManager?.initialize(context, context.mainLooper, directActionListener)
        channel?.also {
            broadcastReceiver =
            ...
            try {
                ...
                while (keepRunning) {
                    ...
                    if (msg == "connected") {
                        ...
                        if(groupcreator){
                            val intent = Intent(context, ActivityReceiveData::class.java)
                            ...
                            val intent = Intent(context, ActivitySendData::class.java)
                            intent.putExtra("owner",groupOwner)
                            (context as Activity).startActivityForResult(
                                intent,
                                MyConstants.FINISH_ACTIVITY_CODE
    ...
    fun resumed() {
        ...
        broadcastReceiver?.let {
            if(context!=null){
                context?.registerReceiver(broadcastReceiver, P2PBroadcastReceiver.getIntentFilter())
                keepRunning = true
            }
        }
    }
