...
class HSReceivingServer(var context: Context) {
    ...
    private var callBacksHS: HSServerCallBacks? = null

    var transferClassHS: HSDetailsInfoToTransferClass? = null

    var inputStream: InputStream? = null
    var outputStream: OutputStream? = null

    init {
        callBacksHS = context as HSServerCallBacks
        try {
            /*server = ServerSocket()
        ...
        private fun listenForIncomingConnections() {
            try {
                ...
                socket?.let { socket1 ->
                    ...
                    transferClassHS = objectInputStream.readObject() as HSDetailsInfoToTransferClass
                    callBacksHS?.receivedTransferInfo(transferClassHS!!)

                    HSMyConstants.FILES_TO_SHARE.addAll(objectInputStream.readObject() as ArrayList<HSFileSharingModel>)
    ...
    private fun receiveFile(HSFileClass: HSFileSharingModel) {
        val trgFile = getTargetFile(HSFileClass)
        val fileOutputStream = trgFile.outputStream()
        var len = -1
        val buffer = ByteArray(HSMyConstants.byteArraySize)
        ...
        callBacksHS?.updateView(transferClassHS)
        if (HSFileClass.fileType == HSMyConstants.FILE_TYPE_PICS) {
            MediaScannerConnection.scanFile(
                context.applicationContext, arrayOf("""${HSFileClass.filePath}"""), arrayOf("image/*")
            ) { path: String, uri: Uri ->
            }
        } else if (HSFileClass.fileType == HSMyConstants.FILE_TYPE_VIDEOS) {
            MediaScannerConnection.scanFile(
    ...
    private fun getTargetFile(fInfo: HSFileSharingModel): File {
        val fileInfo = fInfo

        var rootPath: String = ""
        if (fInfo.fileType == HSMyConstants.FILE_TYPE_APPS) {
