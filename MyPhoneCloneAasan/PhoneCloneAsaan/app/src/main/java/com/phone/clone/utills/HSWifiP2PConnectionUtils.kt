...
import com.phone.clone.interfaces.HSDirectCallBack

import com.phone.clone.interfaces.HSWifiP2PCallBacks
import android.os.Handler;
import android.system.ErrnoException
import com.phone.clone.activity.HSActivityReceive
import com.phone.clone.activity.HSActivitySendData
import java.io.*
import java.lang.Exception
import java.net.*


class HSWifiP2PConnectionUtils(var context: Context) {
    private var wifiP2pManager: WifiP2pManager? = null
    val wifiP2PActionListener = context as HSWifiP2PCallBacks
    ...
    val HSDirectActionListener: HSDirectCallBack = object :
        HSDirectCallBack {
            ...
            try {
                ...
                while (keepRunning) {
                    ...
                    if (msg == "connected") {
                        ...
                        if(groupcreator){
                            val intent = Intent(context, HSActivityReceive::class.java)
                            ...
                            val intent = Intent(context, HSActivitySendData::class.java)
                            intent.putExtra("owner",groupOwner)
                            (context as Activity).startActivityForResult(
                                intent,
                                HSMyConstants.FINISH_ACTIVITY_CODE
    ...
    fun resumed() {
        ...
        broadcastReceiver?.let {
            if(context!=null){
                context?.registerReceiver(broadcastReceiver, HSP2PBroadcastReceiver.getIntentFilter())
                keepRunning = true
            }
        }
    }
    ...
    fun connect(mWifiP2pDevice: WifiP2pDevice) {
        ...
        if (mWifiP2pDevice?.deviceAddress != null) {
            ...
            wifiP2pManager?.connect(channel, config, object : WifiP2pManager.ActionListener {
                override fun onSuccess() {
                    Log.d(HSMyConstants.TAG, "Connect onSuccess")


                }

                override fun onFailure(reason: Int) {
                    Log.d(HSMyConstants.TAG, "Failure")
