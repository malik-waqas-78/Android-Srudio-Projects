...
import com.phone.clone.modelclasses.HSCalendarEventsModel
import java.io.BufferedReader
import java.io.FileReader
import java.io.IOException
import java.util.*

class HSActivityTransferDone : AppCompatActivity() {
...
/*
        ...
        isCalllogs = intent.getBooleanExtra(HSMyConstants.CALLLOGS, false)
        isCalendar = intent.getBooleanExtra(HSMyConstants.CALENDAR, false)
        isContacts = intent.getBooleanExtra(HSMyConstants.CONTACTS, false)
        isPictures = intent.getBooleanExtra(HSMyConstants.PICS, false)
        isVideos = intent.getBooleanExtra(HSMyConstants.VIDEOS, false)
        isApps = intent.getBooleanExtra(HSMyConstants.APPS, false)
        isAudios = intent.getBooleanExtra(HSMyConstants.Audios, false)
        isDocs = intent.getBooleanExtra(HSMyConstants.Docments, false)
    ...
    fun storeOtherData(m: Int) {
        ...
        if (isPictures) {
            ...
            }, multiplier * HSMyConstants.animationTime)
            multiplier++
        }
        if (isVideos) {
            Handler().postDelayed({
    ...
    fun saveContacts() {
        val intent = Intent(this, HSActivityStoreContact::class.java)
        startActivityForResult(intent, HSMyConstants.STORE_CONTACTS)
    }


    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        if (requestCode == HSMyConstants.STORE_CONTACTS) {

            if (data!!.hasExtra(HSMyConstants.isContactsSaved)) {
                isContactsSaved = data.getBooleanExtra(HSMyConstants.isContactsSaved, false)
            }
            ...
            if (isApps) {
                Handler().postDelayed({
                    showSaveAppsDialogue()
                }, 1 * HSMyConstants.animationTime)
            } else {
                if (isAudios) {
                    Handler().postDelayed({
                        findViewById<CardView>(R.id.cl_audios).visibility = View.VISIBLE
                    }, 2 * HSMyConstants.animationTime)
                }
                if (isDocs) {
                    Handler().postDelayed({
                        findViewById<CardView>(R.id.cl_docs).visibility = View.VISIBLE
        ...
        } else if (requestCode == HSMyConstants.INSTALL_APPS) {
    ...
    fun showSaveContactsDialogue() {
        ...
        negative.setOnClickListener {
            ...
            if (isApps) {
                Handler().postDelayed({
                    showSaveAppsDialogue()
                }, m++ * HSMyConstants.animationTime)
            } else {
                if (isAudios) {
                    android.os.Handler().postDelayed({
                        findViewById<CardView>(com.phone.clone.R.id.cl_audios).visibility =
                            android.view.View.VISIBLE
                    }, m++ * com.phone.clone.constants.HSMyConstants.animationTime)
    ...
    fun showSaveAppsDialogue() {
        ...
        positive.setOnClickListener {
            ...
            val intent = Intent(this, HSActivityInstallApps::class.java)
            startActivityForResult(intent, HSMyConstants.INSTALL_APPS)
        }
        negative.setOnClickListener {
            dialog.dismiss()
    ...
    private fun rateIntentForUrl(url: String): Intent? {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            intent = Intent(
                Intent.ACTION_VIEW,
                Uri.parse(
                    url + Objects.requireNonNull(this@HSActivityTransferDone).packageName
                )
            )
        }
        ...
        override fun doInBackground(vararg p0: Void?): Void? {
            ...
            try {
                ...
                while (line != null) {
                    ...
                    if (tokens.size > 0) {
                        var duration =
                            if (tokens[HSCalendarEventsModel.DURATION_INDEX] == null || tokens[HSCalendarEventsModel.DURATION_INDEX].contains(
                                    "null"
                                )
                            ) {
                                "PT1H"
                            } else {
                                tokens[HSCalendarEventsModel.DURATION_INDEX]
                            }
                        val values = ContentValues()
                        values.put(
                            CalendarContract.Events.DESCRIPTION,
                            tokens[HSCalendarEventsModel.DESCRIPTION_INDEX]
                        )
                        values.put(
                            CalendarContract.Events.DTSTART,
                            tokens[HSCalendarEventsModel.START_INDEX]
                        )
                        values.put(
                            CalendarContract.Events.EVENT_TIMEZONE,
                            tokens[HSCalendarEventsModel.TIME_ZONE_INDEX]
                        )
                        values.put(
                            CalendarContract.Events.TITLE,
                            tokens[HSCalendarEventsModel.TITLE_INDEX]
                        )
