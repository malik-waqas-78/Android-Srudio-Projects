...
class HSActivityStoreContact : AppCompatActivity()  {
    ...
    var HSAdapter_Contacts:HSAdapterContacts?=null
    ...
    inner class asyncTask : AsyncTask<Unit,Unit,Unit>(){
        ...
        override fun onPreExecute() {
            super.onPreExecute()
           progressDialog = ProgressDialog(this@HSActivityStoreContact)
        ...
        override fun onPostExecute(result: Unit?) {
            ...
            if(this@HSActivityStoreContact.isDestroyed||this@HSActivityStoreContact.isFinishing){
                return
            }
    ...
    inner class LoadContactsFile() : AsyncTask<Void, Void, Void>() {
        ...
        override fun doInBackground(vararg p0: Void?): Void? {
            ...
            try {
                ...
                while (line != null) {
                    ...
                    if (tokens.size > 0) {
                        var contact = HSContactsModel(
                            tokens[HSContactsModel.NAME_INDEX],
                            tokens[HSContactsModel.PHONE_NUMBER_INDEX]
                        )
        ...
        override fun onPostExecute(result: Void?) {
            ...
            if (contactsList.size > 0) {
                ...
                var linearLayoutManager = LinearLayoutManager(this@HSActivityStoreContact)
                linearLayoutManager.orientation = LinearLayoutManager.VERTICAL
                rc_contacts?.layoutManager = linearLayoutManager
                //create HSAdapter
                HSAdapter_Contacts = HSAdapterContacts(this@HSActivityStoreContact, contactsList)
...
//                "Are you sure you want to Allow permissions?")
        positive.setOnClickListener {
            dialog.dismiss()
            val intent=Intent()
            intent.putExtra(HSMyConstants.isContactsSaved,isContactsSaved)
            setResult(HSMyConstants.STORE_CONTACTS,intent)
            finish()
        }
