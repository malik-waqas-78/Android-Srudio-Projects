 ...
 */
public class AudioRecorderOKRA {
   ...
   */
  public AudioRecorderOKRA(ParcelFileDescriptor file) {
        ...
        new Thread() {
          ...
          public void run() {
            ...
            if (record.getState() != AudioRecord.STATE_INITIALIZED) {
              Log.w("waqas", "Failed to start recording");
              mAlive = false;
              return;
            }
            ...
            try {
              while (isRecording()) {
                ...
                if (len >= 0 && len <= buffer.size) {
                  ...
                  Log.w("waqas", "Unexpected length returned: " + len);
                }
              }
            } catch (IOException e) {
              Log.e("waqas", "Exception with recording stream", e);
            } finally {
              stopInternal();
              try {
                record.stop();
              } catch (IllegalStateException e) {
                Log.e("waqas", "Failed to stop AudioRecord", e);
              }
              record.release();
            }
          }
  ...
  private void stopInternal() {
    ...
    try {
      ...
      Log.e("waqas", "Failed to close output stream", e);
    }
  }
  ...
  private static class Buffer extends AudioBuffeOKRA {
    @Override
