...
import com.zak.clone.zakinterfaces.ZakDirectCallBack

import com.zak.clone.zakinterfaces.ZakWifiP2PCallBacks
import android.os.Handler;
import android.system.ErrnoException
import com.zak.clone.zakactivity.ZakActivityReceive
import com.zak.clone.zakactivity.ZakActivitySendData
import java.io.*
import java.lang.Exception
import java.net.*


class ZakWifiP2PConnectionUtils(var context: Context) {
    private var wifiP2pManager: WifiP2pManager? = null
    val wifiP2PActionListener = context as ZakWifiP2PCallBacks
    ...
    val HSDirectActionListener: ZakDirectCallBack = object :
        ZakDirectCallBack {
            ...
            try {
                ...
                while (keepRunning) {
                    ...
                    if (msg == "connected") {
                        ...
                        if(groupcreator){
                            val intent = Intent(context, ZakActivityReceive::class.java)
                            ...
                            val intent = Intent(context, ZakActivitySendData::class.java)
                            intent.putExtra("owner",groupOwner)
                            (context as Activity).startActivityForResult(
                                intent,
                                ZakMyConstants.FINISH_ACTIVITY_CODE
    ...
    fun resumed() {
        ...
        broadcastReceiver?.let {
            if(context!=null){
                context?.registerReceiver(broadcastReceiver, ZakP2PBroadcastReceiver.getIntentFilter())
                keepRunning = true
            }
        }
    }
    ...
    fun connect(mWifiP2pDevice: WifiP2pDevice) {
        ...
        if (mWifiP2pDevice?.deviceAddress != null) {
            ...
            wifiP2pManager?.connect(channel, config, object : WifiP2pManager.ActionListener {
                override fun onSuccess() {
                    Log.d(ZakMyConstants.TAG, "Connect onSuccess")


                }

                override fun onFailure(reason: Int) {
                    Log.d(ZakMyConstants.TAG, "Failure")
