...
public class    VoiceAdapter extends RecyclerView.Adapter<VoiceAdapter.ViewHolder> {
    ...
    public VoiceAdapter(Context context, ArrayList<ImagesModelClass> voicenotes) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        ((ViewHolder) holder).image_paly_btn.setOnClickListener((View.OnClickListener) v -> {
            if (mediaPlayer.isPlaying()) {
               ...
               mediaPlayer.pause();
                ...
                mediaPlayer.start();
                /*((ViewHolder) holder).image_play_btn.setImageResource(R.drawable.ic_pause_audio);*/
                ((ViewHolder)holder).seekbar();
            }
        });
        ((ViewHolder) holder).prepareMediaPlayer();
        ((ViewHolder)holder).player_seekBar.setOnTouchListener((v, event) -> {
        ...
        mediaPlayer.setOnBufferingUpdateListener((mp, i) -> ((ViewHolder)holder).player_seekBar.setSecondaryProgress(i));
        ((ViewHolder)holder).btn_share.setOnClickListener(v -> {
            Intent intent = new Intent(Intent.ACTION_SEND);
            intent.setType("audio/*");
            intent.putExtra(Intent.EXTRA_STREAM, voicenotes.get(position).getPath());
            context.startActivity(Intent.createChooser(intent, "Share Voice Using "));
        });
