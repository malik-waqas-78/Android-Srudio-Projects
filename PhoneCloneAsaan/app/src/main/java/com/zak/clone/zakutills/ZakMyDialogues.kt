package com.zak.clone.zakutills
...
import com.zak.clone.zakactivity.ZakActivityChooseDevice
import com.zak.clone.zakinterfaces.ZakDialogueClickListener


class ZakMyDialogues(var activity: Activity) {
    var dialogueClickListner = activity as ZakDialogueClickListener
...
//                "Are you sure you want to Allow permissions?")
    ...
    fun decodingData() {
        decodingDialogue = Dialog(activity).apply {
            ...
            setContentView(R.layout.zak_db_decoding_data)
...
//            dialogueClickListner.negativeRunTimeButton()
    ...
    fun tuneWifiOFF(state: Boolean) {
        if (!activity.isFinishing) {
            val dialog = Dialog(activity)
            dialog.setCancelable(true)
            dialog.setContentView(R.layout.zak_normal_dialogue_view)
            dialog.getWindow()!!.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
            val positive: Button = dialog.findViewById(R.id.btn_yes)
            positive.text = "reTry"
//        positive.setText("I'm sure")
...
//                "Are you sure you want to Allow permissions?")
    ...
    companion object {
        fun updateToNewVersion(
            ...
            title: String,
            msg: String,
            clickListener: ZakActivityChooseDevice.OpenSettingsForPermissions
        ) {
            val dialog = Dialog(context)
            dialog.setCancelable(false)
            dialog.setContentView(R.layout.zak_update_dialogue)
            dialog.getWindow()!!.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
    ...
    fun showRational(
        ...
        title: String,
        msg: String,
        callBacks: ZakMyPermissions.ExplanationCallBack
    ) {
        if (!activity.isFinishing) {
            val dialog = Dialog(activity)
            dialog.setCancelable(false)
            dialog.setContentView(R.layout.zak_db_permissions_rationals)
            dialog.getWindow()!!.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
            val btn_allow: Button = dialog.findViewById(R.id.btn_allow)
            val btn_dontAllow: Button = dialog.findViewById(R.id.btn_dont_allow)
        ...
        fun showPermissionsRequired(activity: Activity, msg: String,callBacks:ZakMyPermissions.PermissionsDeniedCallBack) {
            if (!activity.isFinishing) {
                val dialog = Dialog(activity)
                dialog.setCancelable(false)
                dialog.setContentView(R.layout.zak_db_permissions_rationals)
    ...
    fun showRational(
        ...
        title: String,
        msg: String,
        callBacks: ZakMyPermissions.RationalCallback
    ) {
        if (!activity.isFinishing) {
            val dialog = Dialog(activity)
            dialog.setCancelable(false)
