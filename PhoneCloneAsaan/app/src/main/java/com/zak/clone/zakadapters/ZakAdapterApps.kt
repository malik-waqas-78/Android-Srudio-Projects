...
class ZakAdapterApps(var context: Context, var appSrcDir: ArrayList<ZakAppsModel>) :
    RecyclerView.Adapter<ZakAdapterApps.ApplicationViewHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ApplicationViewHolder {
        return ApplicationViewHolder(
            LayoutInflater.from(context).inflate(R.layout.zak_row_applications, parent, false)
        )
    }
    ...
    override fun onBindViewHolder(holder: ApplicationViewHolder, position: Int) {
        ...
        Glide.with(context).load(icon).into(holder.iv_appIcon)
            .onLoadStarted(context.getDrawable(R.drawable.ic_apps))
    ...
    fun installThisApp(label: String) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            var contentUri: Uri? =null
                try{
                    contentUri=FileProvider.getUriForFile(
                        context,
                        "com.phone.clone.switchphone.data.transfer.oldtonewphone.fileprovider",
                        file
                    )
            ...
            (context as Activity).startActivityForResult(install, ZakMyConstants.INSTALL_APPS)
            // finish()
        } else {
            val install = Intent(Intent.ACTION_VIEW)
