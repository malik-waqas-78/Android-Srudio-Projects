...
class CAPPAdapterApps(var context: Context, var appSrcDir: ArrayList<CAPPAppsModel>) :
    RecyclerView.Adapter<CAPPAdapterApps.ApplicationViewHolder>() {


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ApplicationViewHolder {
        return ApplicationViewHolder(
            LayoutInflater.from(context).inflate(R.layout.row_applications_capp, parent, false)
        )
    }
    ...
    override fun onBindViewHolder(holder: ApplicationViewHolder, position: Int) {
        ...
        Glide.with(context).load(icon).into(holder.iv_appIcon)
            .onLoadStarted(context.getDrawable(R.drawable.apps))
    ...
    fun installThisApp(label: String) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            var contentUri: Uri? =null
                try{
                    contentUri=FileProvider.getUriForFile(
                        context,
                        "com.smartswitch.phoneclone.fileprovider",
                        file
                    )
            ...
            (context as Activity).startActivityForResult(install, CAPPMConstants.INSTALL_APPS)
            // finish()
        } else {
            val install = Intent(Intent.ACTION_VIEW)
