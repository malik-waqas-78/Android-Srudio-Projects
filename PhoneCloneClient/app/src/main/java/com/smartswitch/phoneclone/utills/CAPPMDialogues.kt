...
import com.smartswitch.phoneclone.activities.CAPPActivityChooseDevice
import com.smartswitch.phoneclone.interfaces.CAPPDialogueClickListener


class CAPPMDialogues(var activity: Activity) {
    var dialogueClickListner = activity as CAPPDialogueClickListener
...
//                "Are you sure you want to Allow permissions?")
    ...
    fun decodingData() {
        decodingDialogue = Dialog(activity).apply {
            ...
            setContentView(R.layout.db_decoding_data_capp)
...
//            dialogueClickListner.negativeRunTimeButton()
    ...
    fun tuneWifiOFF(state: Boolean) {
        if (!activity.isFinishing) {
            val dialog = Dialog(activity)
            dialog.setCancelable(true)
            dialog.setContentView(R.layout.normal_dialogue_view_capp)
            dialog.getWindow()!!.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
            val positive: Button = dialog.findViewById(R.id.btn_yes)
            positive.text = "reTry"
//        positive.setText("I'm sure")
...
//                "Are you sure you want to Allow permissions?")
    ...
    companion object {
        fun updateToNewVersion(
            ...
            title: String,
            msg: String,
            clickListener: CAPPActivityChooseDevice.OpenSettingsForPermissions
        ) {
            val dialog = Dialog(context)
            dialog.setCancelable(false)
            dialog.setContentView(R.layout.update_dialogue_capp)
            dialog.getWindow()!!.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
    ...
    fun showRational(
        ...
        title: String,
        msg: String,
        callBacks: CAPPMPermissions.ExplanationCallBack
    ) {
        if (!activity.isFinishing) {
            val dialog = Dialog(activity)
            dialog.setCancelable(false)
            dialog.setContentView(R.layout.db_permissions_rationals_capp)
            dialog.getWindow()!!.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
            val btn_allow: Button = dialog.findViewById(R.id.btn_allow)
            val btn_dontAllow: Button = dialog.findViewById(R.id.btn_dont_allow)
        ...
        fun showPermissionsRequired(activity: Activity, msg: String,callBacks:CAPPMPermissions.PermissionsDeniedCallBack) {
            if (!activity.isFinishing) {
                val dialog = Dialog(activity)
                dialog.setCancelable(false)
                dialog.setContentView(R.layout.db_permissions_rationals_capp)
    ...
    fun showRational(
        ...
        title: String,
        msg: String,
        callBacks: CAPPMPermissions.RationalCallback
    ) {
        if (!activity.isFinishing) {
            val dialog = Dialog(activity)
            dialog.setCancelable(false)
