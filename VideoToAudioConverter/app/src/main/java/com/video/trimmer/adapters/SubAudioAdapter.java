...
public class SubAudioAdapter  extends RecyclerView.Adapter<SubAudioAdapter.ViewHolder> implements Filterable {
    Context context;
    ArrayList<AudioModal> audioModals=new ArrayList<>();
    ArrayList<AudioModal> audioModalexample=new ArrayList<>();
    OnAudioClickInterface onAudioClickInterface;

    public SubAudioAdapter(Context context, ArrayList<AudioModal> audioModals) {
        this.context = context;
        this.audioModals = audioModals;
        audioModalexample=audioModals;
    }

    public void onAudioItemClicked(OnAudioClickInterface onAudioClickInterface){
        this.onAudioClickInterface=onAudioClickInterface;
    }
    @NonNull
    @NotNull
    @Override
    public SubAudioAdapter.ViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.audio_item, parent, false);
        SubAudioAdapter.ViewHolder viewHolder = new SubAudioAdapter.ViewHolder( view);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull SubAudioAdapter.ViewHolder holder, int position) {
...
//               }
        ...
        holder.frameLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                onAudioClickInterface.onAudioItemClick(audioModals.get(position).getOriginalPath(),audioModals.get(position).getName(),audioModals.get(position).getDuration(),(audioModals.get(position).getSize()/1024));
            }
        });
    }
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                if (constraint == null || constraint.length() == 0) {
                    filteredList.addAll(audioModalexample);
                } else {
                    String filterPattern = constraint.toString();
                    for (AudioModal item : audioModalexample) {
