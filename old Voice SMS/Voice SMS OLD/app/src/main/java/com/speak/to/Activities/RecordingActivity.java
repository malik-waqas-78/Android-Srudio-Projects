        ...
        timer = new CountDownTimer(maxTime, 1000) {
            ...
            public void onTick(long timeInMilliSeconds) {
                if (timeInMilliSeconds < 0) {
                    stopRecording();
                    startActivity(new Intent(context, RecordingListActivity.class));
                }
                    ...
                    if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                            if (!shouldShowRequestPermissionRationale(Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                                showMessageOKCancel("You need to allow write access to storage to save your audio",
                                        new DialogInterface.OnClickListener() {
                                            @Override
                                            public void onClick(DialogInterface dialog, int which) {
                                                Intent intent = new Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS);
                    ...
                    if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                            if (!shouldShowRequestPermissionRationale(Manifest.permission.READ_EXTERNAL_STORAGE)) {
                                showMessageOKCancel("You need to allow read access to storage to save your audio",
                                        new DialogInterface.OnClickListener() {
                                            @Override
                                            public void onClick(DialogInterface dialog, int which) {
                                                Intent intent = new Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS);
