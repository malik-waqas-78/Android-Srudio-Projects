...
import com.screen.mirror.Adapters.Images.GridImagesAdapter;
...
import com.screen.mirror.databinding.ActivityImagesGridShowCaBinding;
...
public class ImagesGridShowActivity_OKRA extends AppCompatActivity implements InterAdHelper_OKRA.Companion.AdLoadCallBack{
    ActivityImagesGridShowCaBinding binding;
    Context context;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityImagesGridShowCaBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        showInterstital();
        ///adloading
        if(InterAdHelper_OKRA.isAppInstalledFromPlay(ImagesGridShowActivity_OKRA.this)) {
            initSdkandLoadBottomBannerAd();
        }
        context = ImagesGridShowActivity_OKRA.this;
        sharedPrefHelperCA = new SharedPrefHelperCA(context);
        Intent incomingIntent = getIntent();

        if (incomingIntent.hasExtra(FOLDER_NAME)){
            binding.toolbarImagesGrid.setTitle(getIntent().getStringExtra(FOLDER_NAME));
        } else {
            binding.toolbarImagesGrid.setTitle(getResources().getString(R.string.images));
        }
        setSupportActionBar(binding.toolbarImagesGrid);
        ...
        if (rootFolderPath != null) {
            ...
            if (list.size() > 0){
                binding.gridRecyclerViewImages.setLayoutManager(new GridLayoutManager(context,3));
                binding.gridRecyclerViewImages.setAdapter(new GridImagesAdapter(context, list));
            }
        }
    }
    ...
    private void showInterstital() {
            ...
            if(InterAdHelper_OKRA.isAppInstalledFromPlay(ImagesGridShowActivity_OKRA.this)) {
                InterAdHelper_OKRA.showAdmobIntersitial(ImagesGridShowActivity_OKRA.this);
            }
    ...
    public void adClosed() {
        if(InterAdHelper_OKRA.isAppInstalledFromPlay(ImagesGridShowActivity_OKRA.this)) {
            InterAdHelper_OKRA.loadAdmobInterstitial(ImagesGridShowActivity_OKRA.this);
        }
    }
