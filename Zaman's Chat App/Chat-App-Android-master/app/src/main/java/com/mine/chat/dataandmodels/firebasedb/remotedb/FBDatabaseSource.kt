...
class FirebaseDataSource {
    ...
    private fun <T> attachValueListenerToBlock(resultClassName: Class<T>, b: ((ModelResult<T>) -> Unit)): ValueEventListener {
        return (object : ValueEventListener {
            override fun onCancelled(error: DatabaseError) { b.invoke(ModelResult.Error(error.message)) }

            override fun onDataChange(snapshot: DataSnapshot) {
                if (wrapSnapshotToClass(resultClassName, snapshot) == null) {
                    b.invoke(ModelResult.Error(msg = snapshot.key))
                } else {
                    b.invoke(ModelResult.Success(wrapSnapshotToClass(resultClassName, snapshot)))
                }
            }
        })
    }

    private fun <T> attachValueListenerToBlockWithList(resultClassName: Class<T>, b: ((ModelResult<MutableList<T>>) -> Unit)): ValueEventListener {
        return (object : ValueEventListener {
            override fun onCancelled(error: DatabaseError) { b.invoke(ModelResult.Error(error.message)) }
            override fun onDataChange(snapshot: DataSnapshot) {
                b.invoke(ModelResult.Success(wrapSnapshotToArrayList(resultClassName, snapshot)))
            }
        })
    }

    private fun <T> attachChildListenerToBlock(resultClassName: Class<T>, b: ((ModelResult<T>) -> Unit)): ChildEventListener {
    ...
    fun updateLastMessage(chatID: String, myMessage: MyMessage) {
        refToPath("chats/$chatID/lastMessage").setValue(myMessage)
    }
    ...
    fun pushNewMessage(messagesID: String, myMessage: MyMessage) {
        refToPath("messages/$messagesID").push().setValue(myMessage)
    }
    ...
    fun <T> attachUserObserver(resultClassName: Class<T>, userID: String, refObs: FirebaseReferenceValueObserver, b: ((ModelResult<T>) -> Unit)) {
        val listener = attachValueListenerToBlock(resultClassName, b)
        refObs.start(listener, refToPath("users/$userID"))
    }

    fun <T> attachUserInfoObserver(resultClassName: Class<T>, userID: String, refObs: FirebaseReferenceValueObserver, b: ((ModelResult<T>) -> Unit)) {
        val listener = attachValueListenerToBlock(resultClassName, b)
    ...
    fun <T> attachUserNotificationsObserver(resultClassName: Class<T>, userID: String, firebaseReferenceValueObserver: FirebaseReferenceValueObserver,
        b: ((ModelResult<MutableList<T>>) -> Unit)
    ) {
    ...
    fun <T> attachMessagesObserver(resultClassName: Class<T>, messagesID: String, refObs: FirebaseReferenceChildObserver, b: ((ModelResult<T>) -> Unit)) {
        val listener = attachChildListenerToBlock(resultClassName, b)
        refObs.start(listener, refToPath("messages/$messagesID"))
    }

    fun <T> attachChatObserver(resultClassName: Class<T>, chatID: String, refObs: FirebaseReferenceValueObserver, b: ((ModelResult<T>) -> Unit)) {
        val listener = attachValueListenerToBlock(resultClassName, b)
